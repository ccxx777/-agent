"""生成合同审查、报告问答与通用聊天三条线路的数据流图。

这是一张基于当前代码实现的 data-flow 图，不是目标架构图。图中用红色标注
“工资是多少”无法回答的当前断点：报告问答读取的是报告摘要，而不是确认事实表
中的 ``effective_value``；通用聊天则不会读取用户私有合同。
"""
...
def build_svg() -> str:
    ...
    add(lines, '<title id="title">合同审查、报告问答与通用聊天数据流</title>')
    add(
        lines,
        '<desc id="desc">对比合同上传审查、针对报告提问和直接开启聊天三条线路，并标注工资事实无法在报告问答中被读取的当前断点。</desc>',
    )
    add(
        lines,
    ...
    text(lines, 60, 53, "三条对话线路：为什么“工资是多少”会回答不出来？", size=29, weight=750)
    text(
        lines,
        60,
        86,
        "当前实现的数据流。红色断点表示：报告问答拿到了报告摘要，却没有拿到确认事实表中的工资 effective_value。",
        size=14,
        fill="#64748b",
    )
    # Common auth strip.
    ...
    text(lines, 88, 135, "三条线路共用：Authorization Bearer → get_current_user → user_id 注入 → Service/Repository 按 user_id、session_id 做归属检查", size=13, fill="#3730a3", weight=650)
    ...
    lane(lines, 535, 390, "#faf7ff", "#ddd6fe", "2  点击“针对报告提问” → 报告专属会话", "#6d28d9", "lane-report-chat")
    lane(lines, 980, 320, "#fffbeb", "#fed7aa", "3  直接开启聊天 → 通用 / 法律知识问答", "#b45309", "lane-general-chat")
    ...
    card(lines, "r-frontend", 355, 590, "前端 ChatPage", "mode=contract_review", badge="WEB", fill="#faf5ff", stroke="#a78bfa", status="带 review_id", status_fill="#ede9fe", status_text="#6d28d9", subtitle_2="session_id = 报告会话")
    card(lines, "r-api", 640, 590, "Chat API / ChatService", "校验报告归属 + session_id", badge="API", fill="#faf5ff", stroke="#a78bfa", status="已鉴权", status_fill="#ede9fe", status_text="#6d28d9")
    card(lines, "r-context", 925, 590, "_report_context", "读取 findings / sources / pending", badge="CTX", fill="#fff7ed", stroke="#fb923c", status="当前实现", status_fill="#ffedd5", status_text="#c2410c", subtitle_2="不读取 confirmation.facts")
    card(lines, "r-graph", 1210, 590, "LangGraph", "chatbot → ModeAwareToolNode", badge="G", fill="#f5f3ff", stroke="#8b5cf6", status="工具禁用", status_fill="#ede9fe", status_text="#6d28d9", subtitle_2="contract_review allowlist = ∅")
    card(lines, "r-generate", 1495, 590, "generate_answer", "ANSWER_PROMPT + report_context", badge="LLM", fill="#fff7ed", stroke="#fb923c", status="可能拒答", status_fill="#ffedd5", status_text="#c2410c")
    card(lines, "r-response", 1780, 590, "报告会话回复", "答案或“知识库未找到”", badge="OUT", fill="#fff7ed", stroke="#fb923c", status="用户可见", status_fill="#ffedd5", status_text="#c2410c")
    callout(
        lines,
        "report-gap",
        905,
        760,
        980,
        "当前断点：工资事实没有进入报告问答上下文",
        [
            "合同事实表里有 salary 的 effective_value，但 _report_context 只拼接风险发现、法律依据和待确认问题。",
            "合同原文也没有被直接注入通用 RAG；所以“工资是多少”既查不到 facts，也查不到私有合同文本。",
        ],
    )
    ...
    card(lines, "s-checkpoint", 1690, 1405, "LangGraph Checkpoint", "普通 session / report 专属 thread", badge="MEM", fill="#f5f3ff", stroke="#8b5cf6", status="按 thread_id", status_fill="#ede9fe", status_text="#6d28d9", width=470, height=112)
    ...
    text(lines, 900, 1599, "下一步修复方向：报告问答上下文增加已确认 facts（尤其是 effective_value + evidence），或增加报告事实查询工具；保留 user_id / review_id 归属校验。", size=12, fill="#475569", weight=650)
    add(lines, "</svg>")
    return "\n".join(lines)
