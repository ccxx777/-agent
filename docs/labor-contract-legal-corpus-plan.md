...
- [ ] 每个文档记录 `license_status`、`pii_status` 和 `review_status`；官方法律正文的
      `license_status` 用于记录“来自国家法律法规数据库的公开官方来源”，不是向终端用户
      索取额外授权的产品门禁。
...
维护人已确认 `raw/a_level/` 的 Word/WPS 文件从国家法律法规数据库下载；脚本会校验原始 Word 和标准化 Markdown 的 SHA-256。此来源声明不等于法律专业激活，仍不能跳过适用范围、版本/废止衔接、正文一致性和规则复核；但不要求终端用户另行取得法律文本授权。
