...
- [x] 将“事实加载、事实确认门禁、范围检查、法律检索、规则计算、案例补充、报告表达”拆成清晰的 LangGraph 节点。
- [x] 信息不足时暂停并提出补充问题；事实未确认前不运行法律检索和风险规则。
- [x] 先实现全国通用规则，不加入地方性判断。
- [x] 报告字段固定为：风险事实、风险等级、判定依据、相关条款、可选案例、修改建议、待确认事项、免责声明。
- [x] 新增 `POST /api/contract-reviews/{review_id}/workflow`，复用确认快照，不覆盖原始提取值。
- [x] A/B Collection 通过 `LEGAL_A_COLLECTION`、`LEGAL_B_COLLECTION` 注入；缺库时返回 `partial` 和安全警告。

### 3.7 合同审查 Workflow v0.1 已实现范围

- [x] `ContractReviewWorkflowState` 只在图中传递用户身份、确认事实、脱敏证据和报告结构，不传递原始合同文件。
- [x] 规则卡片覆盖主体、书面合同、期限、试用期、工资、工时/加班、社保、解除终止、经济补偿、培训服务期、竞业、保密和争议证据等 17 个检查主题。
...
## 2026-08-01 统一会话与发布治理开发任务

本轮开发将产品从“一次性合同审查”推进为“可持续对话、可查询报告、可控留存”的闭环。已完成项：

- [x] 合同上传绑定 `session_id`；缺省时自动创建会话。
- [x] 普通问答、法律问答、报告问答复用同一个 LangGraph thread，并校验会话归属。
- [x] PostgreSQL 持久化报告版本，提供报告 JSON 查询、PDF 下载和合同/报告删除接口。
- [x] 默认短留存、长留存显式选择、到期启动清理和公开 Schema 隐私边界检查。
- [x] 从本地 A 级法条生成单法条/跨法条专家评测题草稿。
- [x] 增加安全 Smoke 与法律 Smoke/专家题集联合发布门禁脚本。
...
- [x] 修复默认 JWT 密钥可伪造问题：缺少至少 32 字符 `AUTH_SECRET_KEY` 时拒绝启动。
- [x] 修复非 UUID 会话绕过归属校验、报告模式缺少 `review_id`、法律/报告工具源未隔离等问题。
- [x] 修复周期留存清理、存量到期时间回填、报告版本并发锁和前端报告会话恢复问题。
- [x] 修复多工具调用拦截后 ToolMessage 不完整、到期原件删除失败不可重试问题。
- [ ] 后续补充真实 PostgreSQL Checkpointer 跨轮工具协议集成测试、邀请制访客 Token、审查历史页面和删除审计事件。
