...
def _status(value: str, *, evidence: list[ContractEvidence], confidence: float, raw_value: Any) -> FactStatus:
    ...
    def confirmation_questions(self, facts: Iterable[ContractFact]) -> list[str]:
        ...
        for fact in facts:
            ...
            if fact.status is FactStatus.MISSING:
                question = f"请补充或确认合同中的“{fact.name}”信息。"
            elif fact.status is FactStatus.CONTRADICTED:
                question = f"合同中“{fact.name}”出现不一致，请确认以哪一处内容为准。"
            elif not fact.evidence:
                question = f"请确认“{fact.name}”的原文位置和具体内容。"
            else:
                question = f"请确认合同中的“{fact.name}”是否准确。"
